...
import me.oganesson.emissiver.render.ICustomRenderFast;
import me.oganesson.emissiver.shader.Shaders;
import me.oganesson.emissiver.shader.postprocessing.BloomEffect;
...
public class BloomEffectUtil {
    ...
    public static int renderBloomBlockLayer(RenderGlobal renderglobal, BlockRenderLayer blockRenderLayer, double partialTicks, int pass, Entity entity) {
        ...
        if (Shaders.isOptiFineShaderPackLoaded()) {
            ...
            RENDER_FAST.clear();
            return result;
        } else if (!ConfigHandler.shader.emissiveTexturesBloom) {
        ...
        BloomEffect.strength = (float) ConfigHandler.shader.strength;
        BloomEffect.baseBrightness = (float) ConfigHandler.shader.baseBrightness;
        BloomEffect.highBrightnessThreshold = (float) ConfigHandler.shader.highBrightnessThreshold;
        BloomEffect.lowBrightnessThreshold = (float) ConfigHandler.shader.lowBrightnessThreshold;
        BloomEffect.step = (float) ConfigHandler.shader.step;
        switch (ConfigHandler.shader.bloomStyle) {
            case 0:
