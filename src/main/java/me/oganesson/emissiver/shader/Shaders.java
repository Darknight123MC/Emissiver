package me.oganesson.emissiver.shader;
        ...
        try { // hook optFine. thanks to Scannable.
            ...
            isShaderPackLoaded = () -> {
                try {
                    ...
                    LoggerUtil.logger.warn("Failed reading field indicating whether shaders are enabled. Shader mod integration disabled.");
                    isShaderPackLoaded = null;
                    return false;
                }
            };
            LoggerUtil.logger.info("Find Optifine mod loaded.");
        } catch (ClassNotFoundException e) {
            LoggerUtil.logger.info("No Optifine mod found.");
        } catch (NoSuchFieldException | NoClassDefFoundError e) {
            LoggerUtil.logger.warn("Failed integrating with shader mod. Ignoring.");
        }
    }
...
//        BLACK_HOLE = initShader(BLACK_HOLE, FRAGMENT, "blackhole.frag");
    ...
    public static ShaderObject loadShader(ShaderObject.ShaderType shaderType, String location) {
        try {
            return new ShaderObject(shaderType, readShader(getStream(String.format("/assets/%s/shaders/%s", "emissiver", location)))).compileShader();
        } catch (Exception exception) {
            LoggerUtil.logger.error("error while loading shader {}", location, exception);
        }
        return null;
    }
