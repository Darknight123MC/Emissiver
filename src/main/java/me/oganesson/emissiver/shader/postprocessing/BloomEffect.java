package me.oganesson.emissiver.shader.postprocessing;

import me.oganesson.emissiver.ConfigHandler;
import me.oganesson.emissiver.shader.PingPongBuffer;
import me.oganesson.emissiver.shader.Shaders;
import me.oganesson.emissiver.utils.RenderUtil;
...
public class BloomEffect {
    ...
    public static float strength = (float) ConfigHandler.shader.strength;
    public static float baseBrightness = (float) ConfigHandler.shader.baseBrightness;
    public static float highBrightnessThreshold = (float) ConfigHandler.shader.highBrightnessThreshold;
    public static float lowBrightnessThreshold = (float) ConfigHandler.shader.lowBrightnessThreshold;
    public static float step = (float) ConfigHandler.shader.step;
    ...
    private static void cleanUP(int lastWidth, int lastHeight) {
        if (BUFFERS_D == null || BUFFERS_D.length != ConfigHandler.shader.nMips) {
            if (BUFFERS_D != null) {
            ...
            BUFFERS_D = new Framebuffer[ConfigHandler.shader.nMips];
            BUFFERS_U = new Framebuffer[ConfigHandler.shader.nMips];

            int resX = lastWidth / 2;
            int resY = lastHeight / 2;

            for (int i = 0; i < ConfigHandler.shader.nMips; i++) {
